...
from pong import reward_go_to, compute_action, compute_J_theta
...
def test_compute_J_theta():
    ...
    J_theta = compute_J_theta(w1, w2, rewards, actions, logits, GAMMA, TOTAL_ACTIONS,
                              LAMBD)
    R = tf.constant([1.0257553 ,  0.65275335, -0.09325048, -1.5852581], dtype=tf.float32)
...
def test_compute_J_theta_derivative():
    ...
    with tf.GradientTape() as tape:
        ...
        J_theta = compute_J_theta(w1, w2, rewards, actions, logits.concat(), GAMMA,
                                  TOTAL_ACTIONS, LAMBD)
